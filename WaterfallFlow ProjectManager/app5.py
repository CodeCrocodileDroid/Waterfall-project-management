...
class MainFrame(wx.Frame):
    ...
    def init_ui(self):
        ...
        # Add Columns
        # Column 0: Completed (Toggle)
        col_done = self.task_list.AppendToggleColumn("✔", width=40, mode=wx.dataview.DATAVIEW_CELL_ACTIVATABLE)
        # Column 1: Title
        ...
        # CRITICAL FIX: Bind value changed event to update model when checkboxes are clicked
    ...
    def on_save_project(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                # We do NOT reload or change the project here.
                # This ensures the user stays exactly where they are.
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                with open(pathname, 'r') as file:
                    data = json.load(file)
                    # This replaces the current project in memory
                    self.load_project_data(data, None)
                    self.SetStatusText(f"Loaded {pathname}")
            except IOError as e:
                wx.LogError("Cannot open file '%s'.\n%s" % (pathname, str(e)))
            except json.JSONDecodeError:
                wx.LogError("File '%s' is not a valid JSON project file." % pathname)
    ...
    def load_project_data(self, data, error):
        ...
        # Create new project instance from data
    ...
    def on_item_value_changed(self, event):
        """Handle checkbox toggles to keep data model in sync."""
        item = event.GetItem()
        if not item.IsOk(): return
        ...
        # The value in column 0 is the boolean state
        is_checked = self.task_list.GetValue(row, 0)

        # Update the underlying data object
        obj = self.row_map[row]['obj']
        obj.completed = is_checked

        # Optional: Print to verify
        # print(f"Updated '{obj.title}' completed to {is_checked}")
